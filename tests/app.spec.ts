...
test('twelve worlds, gameplay, pause, reset and help work without browser errors',async({page})=>{
 ...
 await expect(page.getByRole('button',{name:'START'})).toBeEnabled();await expect(page.locator('[data-level]')).toHaveCount(12);
 for(let i=0;i<12;i++){
  await page.locator(`[data-level="${i}"]`).click();await page.getByRole('button',{name:'START'}).click();
...
test('phone layout has no overflow and all worlds restart offline',async({page,context})=>{
 ...
 await page.setViewportSize({width:390,height:844});await page.goto('/');await expect(page.getByRole('button',{name:'START'})).toBeEnabled();
 ...
 await context.setOffline(true);await page.reload();await expect(page.getByRole('button',{name:'START'})).toBeEnabled();
 for(let i=0;i<12;i++){
  await page.locator(`[data-level="${i}"]`).click();await page.getByRole('button',{name:'START'}).click();await expect(page.locator('#timer')).not.toHaveText('00:00');
  await page.getByRole('button',{name:'一時停止'}).click();await page.getByRole('button',{name:'ステージ選択へ'}).click();
 }
...
test('storage corruption does not block startup',async({page})=>{
 await page.addInitScript(()=>localStorage.setItem('gyro-maze-save','{broken'));await page.goto('/');await expect(page.getByRole('button',{name:'START'})).toBeEnabled();
});
...
test('touch rotation, pinch, cancellation, and landscape remain usable',async({page,context})=>{
 ...
 await page.getByRole('button',{name:'START'}).click();await page.setViewportSize({width:844,height:390});
