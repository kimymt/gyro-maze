...
test('three worlds, gameplay, pause, reset and help work without browser errors',async({page})=>{
 const errors:string[]=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('/');
 await expect(page.getByRole('button',{name:'この世界で遊ぶ'})).toBeEnabled();await expect(page.locator('[data-level]')).toHaveCount(3);
 for(let i=0;i<3;i++){
...
test('touch rotation, pinch, cancellation, and landscape remain usable',async({page,context})=>{
 await page.setViewportSize({width:390,height:844});await page.goto('/');await expect(page.getByRole('button',{name:'この世界で遊ぶ'})).toBeEnabled();
 await page.locator('[data-level="2"]').click();const canvas=page.locator('canvas');const rect=await canvas.boundingBox();expect(rect).not.toBeNull();
 ...
 await page.screenshot({path:'test-results/world-03-phone.png',fullPage:true});
