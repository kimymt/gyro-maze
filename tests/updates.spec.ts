...
test('failed installation can retry; updates wait until the selection screen',async({page,context})=>{
 ...
 await expect(page.getByRole('button',{name:'START'})).toBeEnabled();await expect(page.locator('#offline-text')).toHaveText('オフラインで遊べます');
 ...
 await context.setOffline(true);await page.reload();await expect(page.getByRole('button',{name:'START'})).toBeEnabled();await context.setOffline(false);
});
