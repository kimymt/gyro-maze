...
test('stage 10 can be selected by keyboard and stage counts remain correct after returning home',async({page})=>{
 ...
 await expect(page.locator('#scene-index')).toHaveText('10 / 10');
...
test('new-stage records survive startup, a settings save and a subsequent reload',async({page})=>{
 ...
 await expect(page.locator('[data-level="0"]')).toHaveAttribute('aria-pressed','true');
 await expect(page.locator('#scene-index')).toHaveText('01 / 10');
 for(let i=3;i<10;i++)await expect(page.locator(`[data-record="${ids[i]}"]`)).toHaveText(`BEST 01:${i.toString().padStart(2,'0')}`);
});
...
test.describe('expanded stage picker on phones',()=>{
 ...
 for(const viewport of [{width:320,height:568},{width:393,height:852},{width:852,height:393}])test(`${viewport.width}x${viewport.height} keeps ten stage targets and the start button usable`,async({page})=>{
