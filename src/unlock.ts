...
export function openUnlock(dialog:HTMLDialogElement,show:(html:string)=>void,grant:(method:AccessMethod)=>void){
  ...
  function menu(){render('<h2>07〜12を解放する</h2><p>01〜06は無料です。次のどれか一つで、残りの6コースをまとめて遊べます。以前のBESTは残ります。</p><button class="secondary" data-unlock="share">SNSでシェア</button><button class="secondary" data-unlock="donation">寄付する</button><button class="secondary" data-unlock="lightning">Lightningで支援する</button><p class="help-note">いずれも実施後の自己申告です。解放状態はこのブラウザだけに保存します。別端末やデータ削除後は、再送金せずにもう一度申告してください。</p>');}
  async function lightning(){
    render('<h2>Lightningで支援する</h2><p>1 sat以上の好きな金額を、ウォレットから送金してください。送金後の自己申告で07〜12を解放します。</p><label class="payment-field">送金先：開発者<textarea readonly rows="2" data-address aria-label="Lightning送金先"></textarea></label><button class="secondary" data-unlock="copy-address">送金先をコピー</button><canvas class="payment-qr" aria-label="Lightning送金先のQRコード"></canvas><a class="secondary payment-wallet" href="'+LIGHTNING_URL+'">ウォレットを開く</a><button class="primary" data-unlock="sent">送金しました</button><p class="help-note">すでに送金した方は、再送金せず「送金しました」を押してください。GYROのサーバーには送金記録を保存しません。</p>');
    dialog.querySelector<HTMLTextAreaElement>('[data-address]')!.value=LIGHTNING_ADDRESS;
