...
export type AccessMethod='share'|'breathing'|'gratitude'|'donation'|'lightning';
export class Access {
  unlocked=false;
  constructor(){
    ...
    try{const data=JSON.parse(localStorage.getItem(ACCESS_KEY)??'null');this.unlocked=data?.version===1&&['share','breathing','gratitude','donation','lightning'].includes(data.method);}catch{}}
