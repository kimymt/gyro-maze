...
function select(index:number){
  ...
  $('#scene-index').textContent=`${levels[index].number} / 03`;$('#scene-description').textContent=levels[index].description;$('#material-label').textContent=levels[index].materialLabel;
}
function start(){if(!ready)return;dialog.close();physics?.dispose();physics=new Physics(levels[selected]);progress.start();scene.orientation.identity();resetBall();syncMode();$('#play-number').textContent=`WORLD ${levels[selected].number}`;$('#scene').focus();toast('少しずつ傾けて、球を転がそう');}
...
function finish(){
  ...
  showDialog(`<p class="eyebrow">${best?'A NEW PERSONAL BEST':'WORLD COMPLETE'}</p><div class="result-symbol">${icon('check')}</div><h2>たどり着いた。</h2><p>WORLD ${levels[selected].number}</p><div class="result-stats"><div><strong>${formatTime(r.time)}</strong><span>クリアタイム</span></div><div><strong>${Math.round(levels[selected].routeLength)}</strong><span>道のり</span></div></div>${selected<levels.length-1?`<button class="primary" data-action="next">次の世界へ ${icon('arrow')}</button>`:''}<button class="secondary" data-action="restart">もう一度遊ぶ</button><button class="text-button" data-action="select">ステージ選択へ</button>`);
}
