...
const icons={arrow:'<path d="M5 12h14m-6-6 6 6-6 6"/>',pause:'<path d="M9 5v14M15 5v14"/>',reset:'<path d="M3 10a9 9 0 1 1 1 8M3 4v6h6"/>',help:'<circle cx="12" cy="12" r="9"/><path d="M9.5 9a2.5 2.5 0 0 1 5 .5c0 1.8-2.5 2-2.5 4M12 17h.01"/>',chevron:'<path d="m9 5 7 7-7 7"/>',back:'<path d="m14 6-6 6 6 6"/>',check:'<path d="m5 12 4 4L19 6"/>',orbit:'<ellipse cx="12" cy="12" rx="10" ry="5" transform="rotate(-40 12 12)"/><circle cx="12" cy="12" r="3"/>'};
...
root.innerHTML=`<div class="shell"><header class="header"><a class="brand" href="/" aria-label="GYRO ホーム">${icon('orbit')}<span>GYRO<span class="brand-dot">.</span></span></a><span class="header-caption">A LITTLE WORLD IN YOUR HANDS</span><button class="icon-button" data-action="help" aria-label="遊び方">${icon('help')}</button></header>
<main class="workspace"><section class="selection" aria-label="ステージ選択"><div class="intro"><p class="eyebrow">A LITTLE PIECE OF NATURE</p><h1>小さな自然を、<br><span>手のひらで。</span></h1><p class="intro-copy">木と土、水の中を巡る。<br>落ちる心配のない、立体迷路。</p></div><div class="level-heading"><span>SELECT A WORLD</span><span>03 WORLDS</span></div><div class="levels">${levels.map((l,i)=>`<button class="level ${i===0?'selected':''}" data-level="${i}" aria-pressed="${i===0}"><span class="level-number">${l.number}</span><span class="level-copy"><span class="level-name">${l.name}</span><span class="level-subtitle">${l.subtitle}</span><span class="record" data-record="${l.id}"></span></span><span class="difficulty">${l.difficulty}</span>${icon('chevron')}</button>`).join('')}</div><button class="primary start" data-action="start" disabled><span id="start-label">準備しています</span>${icon('arrow')}</button><p class="selection-note">落ちないから、何度でも試せる。</p></section>
<section class="viewer" aria-label="3D迷路"><div class="viewer-grid" aria-hidden="true"></div><div class="scene-caption"><span class="mini-dot"></span><span id="scene-subtitle">WOODLAND CUBE</span><span class="scene-index" id="scene-index">01 / 03</span></div><div class="play-hud" hidden><button class="icon-button" data-action="pause" aria-label="一時停止">${icon('pause')}</button><div class="hud-title"><span id="play-number">WORLD 01</span><strong id="play-name">木漏れ日の箱庭</strong></div><div class="timer" id="timer">00:00</div></div><canvas id="scene" aria-label="迷路をドラッグして回転。2本指でひねると回転、ピンチで拡大縮小。" tabindex="0"></canvas><div class="viewer-bottom"><div class="object-label"><span class="object-line"></span><span id="material-label">WOOD / MOSS</span></div><p id="scene-description">木の外周から、森の内側へ。</p><div class="gesture-hint">${icon('orbit')}<span>ドラッグして回す</span><span class="hint-separator">/</span><span>ピンチで拡大</span></div></div><div class="play-bottom" hidden><div class="progress-row"><span id="checkpoint">スタート地点</span><span id="route-progress">道のり 0%</span></div><div class="play-tools"><button data-action="reset">${icon('reset')}視点リセット</button><button data-action="focus">球に寄る</button><button data-action="restart">再挑戦</button></div></div><div id="toast" role="status" aria-live="polite"></div></section></main>
<footer class="footer"><div class="offline-status"><span class="status-dot"></span><span id="offline-text">オフライン用データを確認中</span><button data-action="retry-offline" hidden>再試行</button><button data-action="update" hidden>更新する</button></div><span class="footer-signature">FIND YOUR BALANCE.</span></footer></div><dialog id="dialog"></dialog>`;
...
function select(index:number){
  ...
  $('#scene-subtitle').textContent=levels[index].subtitle;$('#scene-index').textContent=`${levels[index].number} / 03`;$('#scene-description').textContent=levels[index].description;$('#material-label').textContent=levels[index].materialLabel;
}
function start(){if(!ready)return;dialog.close();physics?.dispose();physics=new Physics(levels[selected]);progress.start();scene.orientation.identity();resetBall();syncMode();$('#play-name').textContent=levels[selected].name;$('#play-number').textContent=`WORLD ${levels[selected].number}`;$('#scene').focus();toast('少しずつ傾けて、球を転がそう');}
function showDialog(html:string){dialog.innerHTML=html;dialog.showModal();}
function pause(){if(progress.phase!=='playing')return;progress.pause();input.clear();accumulator=0;
  showDialog(`<p class="eyebrow">TAKE A BREATH</p><h2>ひと休み。</h2><p>球はそのまま。続きを、いつでも。</p><button class="primary" data-action="resume">ゲームに戻る ${icon('arrow')}</button><button class="secondary" data-action="restart">最初からやり直す</button><button class="text-button" data-action="select">ステージ選択へ</button><label class="quality-setting">描画品質<select id="quality"><option value="auto" ${saved.quality==='auto'?'selected':''}>自動</option><option value="low" ${saved.quality==='low'?'selected':''}>軽量</option></select></label>`);
}
function finish(){
  ...
  showDialog(`<p class="eyebrow">${best?'A NEW PERSONAL BEST':'WORLD COMPLETE'}</p><div class="result-symbol">${icon('check')}</div><h2>たどり着いた。</h2><p>${levels[selected].name}</p><div class="result-stats"><div><strong>${formatTime(r.time)}</strong><span>クリアタイム</span></div><div><strong>${Math.round(levels[selected].routeLength)}</strong><span>道のり</span></div></div>${selected<levels.length-1?`<button class="primary" data-action="next">次の世界へ ${icon('arrow')}</button>`:''}<button class="secondary" data-action="restart">もう一度遊ぶ</button><button class="text-button" data-action="select">ステージ選択へ</button>`);
}
let helpWasPlaying=false;
function help(){helpWasPlaying=progress.phase==='playing';if(helpWasPlaying)progress.pause();input?.clear();showDialog(`<p class="eyebrow">HOW TO PLAY</p><h2>重力を、味方に。</h2><div class="help-steps"><p><b>01</b><span><strong>1本指で回す</strong>ドラッグで迷路を傾ける。指を離すと迷路は止まり、球は転がり続けます。</span></p><p><b>02</b><span><strong>2本指で調整</strong>ひねって回転。ピンチで拡大・縮小。上下を裏返すこともできます。</span></p><p><b>03</b><span><strong>緑のリングへ</strong>金色の中継点を順番に通り、ゴールで球を止めるとクリア。見えない壁が球を守るので、裏返しても落ちません。</span></p></div><p class="help-note">オフラインで遊ぶには、Safariの共有メニューから「ホーム画面に追加」。追加後に開き直し、「オフラインで遊べます」の表示を確認してください。</p><button class="primary" data-action="close-help">わかった ${icon('arrow')}</button>`);}
root.addEventListener('click',e=>{
  ...
  switch(target.dataset.action){
    ...
    case 'close-help':dialog.close();if(helpWasPlaying){progress.resume();last=performance.now();}break;
...
dialog.addEventListener('cancel',e=>{e.preventDefault();if(progress.phase==='paused'){dialog.close();progress.resume();last=performance.now();}else if(progress.phase==='select')dialog.close();});
...
canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();progress.pause();input?.clear();dialog.close();showDialog('<p class="eyebrow">DISPLAY PAUSED</p><h2>描画が停止しました。</h2><p>再読み込みして再開してください。クリア記録は保存されています。</p><button class="primary" data-action="reload">再読み込み</button>');});
...
async function boot(){
  try{
    ...
    requestAnimationFrame(frame);await initPhysics();ready=true;select(selected);$('.start').removeAttribute('disabled');$('#start-label').textContent='この世界で遊ぶ';
  }catch(error){console.error(error);$('#start-label').textContent='読み込めませんでした';toast('再読み込みしてお試しください');showDialog('<p class="eyebrow">UNABLE TO START</p><h2>ゲームを開けませんでした。</h2><p>通信状態とブラウザの3D描画対応を確認し、再読み込みしてください。</p><button class="primary" data-action="reload">再読み込み</button>');}
}
void boot();
