...
// mkdir -p /private/tmp/gyro-stage-baseline
// git archive aff2fc221465dcd2dc6f85dbf5ba774f1de7dea2 | tar -x -C /private/tmp/gyro-stage-baseline
// ln -s /absolute/path/to/gyro-maze/node_modules /private/tmp/gyro-stage-baseline/node_modules
...
const baselineRoot = process.env.GYRO_BASELINE_ROOT || '/private/tmp/gyro-stage-baseline';
const output = process.env.GYRO_STARTUP_OUTPUT || join(root, 'research/stage-expansion-startup-20260920.json');
  ...
  try {
    ...
    Object.assign(row, await page.evaluate(() => {
      ...
      return {...window.__startup, fcp: performance.getEntriesByName('first-contentful-paint')[0]?.startTime ?? null,
        ...
        availableStages: document.querySelectorAll('[data-level]').length,
        selectedStage: document.querySelector('#scene-index')?.textContent ?? null,
...
const touchStartupVerified = runs.every(row => row.orientationListeners === 0 && row.motionListeners === 0 && row.permissionCalls === 0 && row.tiltPageResourceRequests === 0);
const warmCacheVerified = runs.filter(row => row.condition === 'warm').every(row => row.controlledByServiceWorker && row.swReady !== null);
const report = {
  baselineCommit: 'aff2fc221465dcd2dc6f85dbf5ba774f1de7dea2', timestamp: new Date().toISOString(),
  ...
  versionComparison: {before: '0.3.0', after: '0.4.0'},
  interpretation: 'Comparison includes the complete release changes. It does not independently attribute timing differences to lazy geometry generation. Separate catalog tests verify that importing metadata builds no geometry and requesting one stage generates only that stage; geometry-only Node measurements are recorded in stage-expansion-geometry-20260920.json.',
  buildsUnchanged, touchStartupVerified, warmCacheVerified, builds, runs, primers, medians,
...
console.log(JSON.stringify({output, buildsUnchanged, touchStartupVerified, warmCacheVerified, medians, deltas: report.deltas}));
if (!buildsUnchanged || !touchStartupVerified || !warmCacheVerified || runs.some(row => row.error)) process.exitCode = 1;
